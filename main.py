...
def main(argv):
    ...
    writer = SummaryWriter(f'{FLAGS.base_directory}/{FLAGS.logdir}/{timestamp}')
    ...
    model_path = f'{FLAGS.base_directory}/saved_models/{timestamp}'
    while True:
        try:
    ...
    train_dataset, val_dataset, train_indices, val_indices, age_normalization, out_features = make_datasets(
        base_directory=FLAGS.base_directory,
